...
CONFIG_VALUES = {
    ...
    SKIP_PREVIOUSLY_DOWNLOADED: {
        "default": "False",
        "type": bool,
        "arg": "--skip-previously-downloaded",
    },
...
class Config:
    ...
    def get_credentials_location(cls) -> str:
        if cls.get(CREDENTIALS_LOCATION) == "":
            ...
            if sys.platform not in system_paths:
                ...
                credentials_location = PurePath(
                    system_paths[sys.platform] / "credentials.json"
                )
        else:
            credentials_location = PurePath(Path.cwd()).joinpath(
                cls.get(CREDENTIALS_LOCATION)
            )
    ...
    def get_output(cls, mode: str) -> str:
        ...
        # mapping outputs to modes
        outputs = {
            "playlist": "{playlist}/{artist} - {song_name}.{ext}",
            "extplaylist": "{playlist}/{playlist_num} - {artist} - {song_name}.{ext}",
            "liked": "Liked Songs/{artist} - {song_name}.{ext}",
            "single": "{artist}/{album}/{artist} - {song_name}.{ext}",
            "album": "{artist}/{album}/{album_num} - {artist} - {song_name}.{ext}",
        }
        ...
        if cls.get_split_album_discs():
            print("IM NOT SPLITTING ALBUMS IN DISKS")
